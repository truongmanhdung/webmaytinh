...
<head>
    ...
    <style>
        .list-product.d-flex.justify-content-between.w-1200.flex-wrap {
    ...
    <div class="container-fluid">
                    ...
                    <ul class="d-flex">
                        ...
                        if (isset($_COOKIE["user"])) {
                            ...
                            if ($result['permission'] == 'khách hàng') {
                                ...
                                echo "<li><a class=\"fs-13 px-10 font-weight-bold text-white\" href=\"info.php\"><i class=\"mr-5  fas fa-user\"></i>$name</a></li><li><a class=\" fs-13 font-weight-bold text-white\" title=\"Đăng xuất\" href=\"?logoutadmin=\"><i class=\"text-white fas fa-sign-out-alt\"></i></a></li>
        ...
        <main>
            <div class=" w-1200 d-flex justify-content-between">
                <div class="nopadding col-xs-2 col-sm-2 col-md-2 col-lg-2">
                    <ul>
                        <?php
                        foreach (SelectAll("SELECT * FROM category") as $row) {
                        ?>
                            <li class="mb-1"><a href="product.php?id=<?= $row['id'] ?>" style="width:190px;" class="btn 
                            <?php if ($row['id'] == $_GET["id"]) {
                                echo 'btn-danger';
                            } else {
                                echo "btn-primary color border-color";
                            } ?>"><?= $row['name'] ?></a></li>
                        <?php
                        }
                        ?>
                ...
                <div class="p-0 col-xs-10 col-sm-10 col-md-10 col-lg-10">
                    ...
                    if (isset($_GET['search'])) {
                        $search = $_GET['search'];
                        $sqlsearch = "SELECT * FROM products WHERE name LIKE '%$search%'";
                        $result = $conn->prepare($sqlsearch);
                        $result->execute();
                        $count = $result->rowCount();
                        if ($count == 0) {
                            echo '<p class="alert alert-danger font-weight-bold bg-light p-0 " style="border:unset;">Không tìm thấy sản phẩm phù hợp !</p>
                            <a class="btn btn-danger" href="javascript:history.back()"><i class="fas fa-undo-alt"></i> Quay lại</a>
                            ';
                        } else {
                            echo '<p class="alert alert-success bg-light p-0" style="border:unset;">Tìm thấy ' . $count . ' sản phẩm phù hợp</p>';
                            echo '<p class="alert alert-success bg-light p-0" style="border:unset;">Kết quả tìm kiếm cho từ khóa <span class="font-weight-bold">\'' . $search . '\'</span> </p>';
                        };
                    }
                    ?>
                    <div class="list-product d-flex justify-content-between w-1200 flex-wrap">
                        ...
                        if (isset($_GET['id'])) {
                            $cateid = $_GET["id"];
                            foreach (selectAll("SELECT * FROM products WHERE cate_id = $cateid ORDER BY id DESC") as $row) {
                        ?>
                            ...
                            $search = $_GET['search'];
                            foreach (selectAll("SELECT * FROM products WHERE name LIKE '%$search%'ORDER BY id ") as $row) {
                            ?>
                                <div class="box product-item">
                                    <div>
                                        <img class="hover-product" width="160px" src="images/<?= $row['image'] ?>" alt="" title="<?= $row['name'] ?>">
                            ...
                            $view = ($numberpage - 1) * $limit;
                            foreach (selectAll("SELECT * FROM products ORDER BY id LIMIT $view,$limit") as $row) {
                            ?>
                                <div class="box product-item">
                                    <div>
                                        <img class="hover-product" width="160px" src="images/<?= $row['image'] ?>" alt="" title="<?= $row['name'] ?>">
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-center">
                        ...
                        <?php
                            if ($numberpage == $i) {
                                echo 'active';
                            } ?>"><a href="?page=<?= $i ?>" class="page-link" href="#"><?= $i ?></a></li>
                        <?php
                        }
                        ?>
                        <?php
        ...
        <footer>
            ...
            <div id="copyright" class="d-flex justify-content-between">
                <p>Copyright &copy; 2016 nguyenvanlinh.user Designed & Developed by <a href="https://www.facebook.com/NguyenVanLinh.user">Linh</a>
                </p>
                <div id='wave'>
                    <span></span>
                    <span></span>
